...
namespace HeurekaGames
{
    public static class Heureka_WindowStyler
    {
        ...
        public static float HeaderHeight { get; } = 24f;
        ...
        public static void DrawGlobalHeader(Color color, string label, string version = "",
            Action additionHeaderContent = null)
        {
        ...
        public static void DrawCenteredImage(EditorWindow window, Texture image)
        {
            ...
            GUI.Box(
                new Rect(window.position.width * .5f - image.width * .5f,
                    window.position.height * .5f - image.height * .5f, image.width, image.height), image,
                GUIStyle.none);
        }

        public static void DrawCenteredMessage(EditorWindow window, Texture icon, float msgWidth, float msgHeight,
            string messsage)
        {
            if (window == null)
                return;
            ...
            var rectStartPos = new Vector2(window.position.width * .5f - outerBoxSize.x * .5f,
                window.position.height * .5f - outerBoxSize.y * .5f + iconSize.y * .5f);

            EditorGUI.DrawRect(new Rect(rectStartPos.x, rectStartPos.y, outerBoxSize.x, outerBoxSize.y), clr_White);
            EditorGUI.DrawRect(
                new Rect(rectStartPos.x + frameWidth, rectStartPos.y + frameWidth, innerBoxSize.x, innerBoxSize.y),
                clr_dBlue);
            ...
            GUI.Label(new Rect(labelStartPos.x, labelStartPos.y, textWidth, textHeight), lines[0],
